...
# wp.pinMode(pinLed, wp.GPIO.OUTPUT)
# wp.pinMode(pinSw, wp.GPIO.OUTPUT)
...
def Lit(info, i, j):
    ...
    if ((info[i][j])[0] == 1):
        ...
        # wp.digitalWrite(pinLedA[i], wp.GPIO.HIGH)
        # wp.digitalWrite(pinLedC[j], wp.GPIO.LOW)
        GPIO.output(pinLedA[i], GPIO.HIGH)
        GPIO.output(pinLedC[j], GPIO.LOW)
        """
        wp.digitalWrite(pinLedA[i], wp.GPIO.LOW)
        wp.digitalWrite(pinLedC[j], wp.GPIO.LOW)
        """
        for k in rawlist:
            # wp.digitalWrite(pinLedA[k], wp.GPIO.LOW)
            GPIO.output(pinLedA[k], GPIO.LOW)
        for l in columnlist:
            # wp.digitalWrite(pinLedC[l], wp.GPIO.HIGH)
            GPIO.output(pinLedC[l], GPIO.HIGH)
        time.sleep(0.0000000018)
        
    elif( (info[i][j])[0] == 0 ):
        for k in rawlist:
            # wp.digitalWrite(pinLedA[k], wp.GPIO.HIGH)
...
for i in range(raw):
    # wp.pinMode(pinLedA[i], wp.GPIO.OUTPUT)
    GPIO.setup(pinLedA[i], GPIO.OUT)
for i in range(column):
    # wp.pinMode(pinLedC[i], wp.GPIO.OUTPUT)
...
try:
    ...
    for i in range(raw):
        ...
        GPIO.output(pinLedA[i], GPIO.LOW)
    for i in range(column):
        # wp.digitalWrite(pinLedC[i], wp.GPIO.LOW)
        GPIO.output(pinLedC[i], GPIO.LOW)
